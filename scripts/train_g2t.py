...
from g2t.data import SegmentationCsvDataset
from g2t.training import G2TLoss, dice_score
from g2t.utils import seed_everything
...
def main() -> None:
    ...
    dataset = SegmentationCsvDataset(
        cfg["data"]["train_csv"],
    ...
    model = G2T(G2TConfig(**model_cfg)).to(device)
    criterion = G2TLoss(dice_weight=cfg["training"]["dice_weight"])
    ...
    for epoch in range(1, cfg["training"]["epochs"] + 1):
        ...
        print(f"epoch={epoch:03d} loss={total_loss / len(loader):.4f} dice={total_dice / len(loader):.4f}")
